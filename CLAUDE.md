...
| MinIO presign — **기보 뷰어만 남았다** | presigned URL 의 host 는 서명에 포함되므로 만든 뒤 문자열로 못 고친다. app-mvc 는 `storage.s3.endpoint`(서버→MinIO)와 `storage.s3.public-endpoint`(브라우저용, 프로덕션 `https://image.woobeee.com`)로 쪼개 해결했다. **글 본문 이미지와 프로필 이미지 모두** 이 presigned URL 로 브라우저가 MinIO 에 직접 붙고 **버킷은 비공개로 남는다**(서명이 접근을 허가한다). 서명은 AWS SDK 가 아니라 `PresignedUrlFactory` 가 만든다 — SDK 는 `X-Amz-Date` 를 항상 "지금"으로 넣고 클록 주입 수단이 없어 URL 이 요청마다 달라지고, CDN 이 쿼리스트링까지 캐시 키에 넣으므로 방문자마다 다른 오브젝트가 된다(실측: 새 서명은 매번 MISS). 그래서 **서명 시각을 1시간 단위로 내려** 같은 시간대면 동일한 URL 이 나오게 하고, 만료 절벽을 피하려 TTL 은 버킷보다 길게(기본 24시간) 둔다 — 최악의 남은 유효기간 23시간. **남은 것은 app-webflux 의 기보 뷰어**다: 아직 SDK presigner 를 서버용 endpoint 로 쓴다 |
...
| 5B 잔여 (표시상) | `Header` 가 클라이언트 전용 `mounted` 로 렌더를 막아 서버 HTML 에는 없다 — 이제 전 라우트에서 하이드레이션 후 튀어나온다. `/blog` 로 클라이언트 이동할 때 한 프레임 동안 사이드바 토글 없이 그려진다. 죽은 `/products`·`/cart` 가 자체 sticky 헤더를 그려 전역 헤더 뒤로 숨는다. 인증 화면 넷이 `min-h-screen` 이라 64px 헤더 아래에서 살짝 어긋난다 |
...
에서는 제외된다(그래서 위 기본 검증 명령은 계속 445개 그린을 유지한다 — core 6 / app-mvc 138 /
app-webflux 301. 포함해서 돌리면 450개 중 445개 통과, 5개 실패다). 루트 `pom.xml` 의
