...
| 장애물 생성 상한 경계 미검증 | 스폰 확률 상한 테스트가 실제로 상한이 걸리는 틱 750 이 아니라 100000 을 쓴다. 값 자체는 고정돼 있으나 경계는 산술로만 보장된다 |
...
에서는 제외된다(그래서 위 기본 검증 명령은 계속 340개 그린을 유지한다 — core 6 / app-mvc 36 /
app-webflux 298. 포함해서 돌리면 345개 중 340개 통과, 5개 실패다). 루트 `pom.xml` 의
