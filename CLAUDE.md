...
| MinIO presign 호스트 — **기보 뷰어만 남았다** | presigned URL 의 host 는 presigner 를 만들 때 정해지고 서명이 host 를 포함하므로 나중에 문자열로 못 고친다. app-mvc 는 이걸 `storage.s3.endpoint`(서버→MinIO)와 `storage.s3.public-endpoint`(브라우저용, 프로덕션 `https://image.woobeee.com`)로 쪼개 해결했다 — 글 본문 이미지는 이 presigned URL 로 브라우저가 MinIO 에 직접 붙고, **버킷은 비공개로 남는다**(서명이 접근을 허가한다). 프로필 이미지는 presign 을 쓰지 않고 `GET /api/auth/members/{memberId}/profile-image` 가 앱에서 스트리밍한다 — **공개**다(아바타는 남이 봐야 하고 `<img>` 는 Authorization 을 못 보낸다). 키에 UUID 가 있어 열거되지 않고, ETag 로 재검증한다. **남은 것은 app-webflux 의 기보 뷰어**다 — 아직 서버용 endpoint 로 presign 해서 브라우저가 못 여는 URL 이 나올 수 있고, 막히면 원인 없는 일반 네트워크 문구만 뜬다. app-mvc 와 같은 endpoint 분리를 옮기면 된다 |
