...
**이유는 취향이 아니라 실측이다.** 리뷰에서 돌린 뮤테이션 스윕은 이 모듈들 안에 심은 변이를
전부 죽였다(11/11, 생존 0). 컴포넌트 안의 판단에 대해서는 죽일 수 있는 테스트가 **하나도
없다** — 스펙이 전부 `lib/*.test.ts` 이기 때문이다. 즉 판단이 컴포넌트로 넘어가는 만큼 그대로
검증 밖으로 나간다. 리듀서의 조건 하나가 조용히 뒤집혀도 화면은 계속 그려지므로, 컴포넌트에
남은 판단은 틀린 채로 몇 주를 간다. 다음 사람이 "이 정도는 컴포넌트에 둬도 되지 않나" 로
되돌리고 싶어질 텐데, 되돌리는 순간 그만큼을 잃는다.
...
- 재접속은 최대 5회, `500ms` 부터 지수 백오프. 서버가 참가를 거절해 닫은 세션(`rejected`)은
  재시도하지 않는다.
- 봉투는 `{type, seq, payload}`. 타입은 `app-webflux` 의 `ClientMessage`/`ServerMessage` 에서
  필드 단위로 옮겨 왔다 — 서버 계약이 바뀌면 여기가 단일 갱신 지점이다.
