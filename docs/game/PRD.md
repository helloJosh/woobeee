...
| GAME-AC-28 | WebSocket `ERROR` 도 HTTP 봉투와 같은 `GameErrorCode` 카탈로그를 쓴다 — `{status, code, message}` 이고 `code` 는 `game_*` 문자열이다. 카탈로그 밖 예외는 `game_unexpected` 로 뭉개고 예외 메시지를 싣지 않는다. 참가 인증에 실패한 세션은 아직 방 허브를 구독하지 않으므로, 닫히기 전에 그 프레임을 세션에 직접 받는다 | `RoomCommandDispatcherTest.aFailedCommandCarriesTheErrorCodeNotJustAMessage`·`anUnexpectedFailureCarriesTheGenericCodeAndLeaksNothing`·`aGameCommandFromANonMemberCarriesTheNotAMemberCode`, `GameWebSocketHandlerTest.aFailedAuthenticationSendsACodedErrorFrameBeforeClosing` |
