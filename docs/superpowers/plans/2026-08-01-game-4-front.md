...
- Produces: a board that fills the available height and stays square, plus the socket wiring for `OMOK_PLACE` / `OMOK_MOVED` / `OMOK_REJECTED` / `GAME_END`.
...
import { createGameSocket, type GameSocket, type ServerMessage } from "@/lib/game-socket"
import type { ParticipantView, RoomStatus } from "@/lib/types"
    ...
    const handleMessage = useCallback((message: ServerMessage) => {
        switch (message.type) {
            case "ROOM_STATE": {
                ...
                setStatus(message.payload.status ?? "WAITING")
                break
            }
            case "GAME_START": {
            ...
            case "OMOK_MOVED": {
                const { x, y, color, nextTurn } = message.payload
                setPlacements((current) => [...current, { x, y, color }])
                setTurnParticipantId(nextTurn || null)
                setNotice(null)
                break
            }
            case "OMOK_REJECTED": {
            ...
            case "ERROR": {
                setNotice(message.payload?.message ?? "요청을 처리하지 못했습니다.")
                break
            }
        }
    }, [])
        ...
        <main className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6 lg:flex-row lg:items-start">
            <div className="flex min-w-0 flex-1 flex-col items-center gap-3">
                ...
                <p className="h-5 text-sm text-muted-foreground">
                    {notice ?? (status === "IN_PROGRESS" ? (myTurn ? "내 차례" : "상대 차례") : "")}
                </p>
            </div>
...
function winnerName(payload: any): string {
    const ranks = payload?.ranks ?? []
    const winner = ranks.find((entry: any) => entry.rank === 1)
...
- Produces: a keyboard-driven grid that sends `DODGE_MOVE` and renders `DODGE_TICK` frames.
...
export default function DodgeRoomPage() {
    ...
    const handleMessage = useCallback((message: ServerMessage) => {
        switch (message.type) {
            ...
            case "GAME_END": {
                setStatus("FINISHED")
                const ranks = message.payload?.ranks ?? []
                const winner = ranks.find((entry: any) => entry.rank === 1)
    ...
    return (
        <main className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6 lg:flex-row lg:items-start">
            <div className="flex min-w-0 flex-1 flex-col items-center gap-3">
                ...
                <p className="h-5 text-sm text-muted-foreground">
                    {notice ?? (status === "IN_PROGRESS" ? "방향키 또는 WASD 로 이동" : "")}
                </p>
            </div>
