...
`Room` 은 단일 스레드처럼 다뤄야 한다. 8인이 동시에 입력을 보내므로 방 상태 변경은 방마다
직렬화한다. 방 전용 단일 스레드 `Scheduler` (`Schedulers.newSingle`)에 상태 변경을 태우는 대신,
방마다 `Sinks.Many<Command>` 입력 큐를 두고 그 스트림을 순차 처리하는 방식을 쓴다 — 틱과 입력이
같은 큐를 지나므로 락 없이 순서가 보장된다.
