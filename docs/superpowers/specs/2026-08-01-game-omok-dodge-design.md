...
게임 HTTP API 의 **컨트롤러가 처리한** 요청의 실패 응답은 app-mvc 와 같은 `ApiResponse` 봉투로
나간다.
...
- WebSocket 경로의 오류는 이 봉투가 아니라 `ERROR` 서버 메시지로 나간다. 모양은 다르지만 **코드는
  같은 카탈로그**를 쓴다 — `{status, code, message}` 이고 `code` 가 `header.message` 와 같은 값이다.
  두 통로가 한 지도로 문구를 찾을 수 있게 하기 위해서다.

**봉투가 닿지 않는 곳** — `@ControllerAdvice` 는 핸들러 **호출**만 감싼다. 따라서 다음 둘은
어드바이스를 거치지 않고 Boot 기본 오류 본문으로 나가고, 프론트는 `code` 를 찾지 못해
`"unknown"` 으로 두고 일반 문구를 띄운다:

- 매칭되는 라우트가 없는 요청의 404 (예: 오타 난 경로).
- `GameAuthWebFilter` 안에서 난 오류 (예: 토큰 검증 중 Redis 장애).

둘 다 비밀이 새지는 않는다(Boot 기본 본문은 스택트레이스를 담지 않는다). 문구가 덜 친절할 뿐인
커버리지 공백이고, 전역 `ErrorWebExceptionHandler` 로 넓히면 위에 적은 대로 WebSocket 핸드셰이크를
같이 삼키게 된다 — 그래서 넓히지 않고 범위를 여기 적어 둔다.
...
오목은 판을 격자로 인코딩하지 않고 착수 목록을 그대로 싣는다. 각 착수가 자기 색을 함께 들고 있어서
(`{x, y, color}`) 클라이언트는 헤더 없이 이 목록만으로 판을 세울 수 있다 — 살아 있는 판 하나를 즉시
그리는 데에는 자기 자신을 설명하는 모양이 맞다.

**이것은 기보 형식이 아니다.** `OmokReplayWriter.toNdjson` 은 색을 헤더의 `players[]` 에 한 번만
선언하고 각 수는 `{t, p, x, y}` 로 적는다. 재생은 헤더부터 순서대로 읽는 것이 전제이므로 그 모양이
맞고, 스냅샷은 헤더가 없으므로 색을 수마다 실어야 한다. 두 형식이 다른 것은 의도된 것이다 — 한쪽을
다른 쪽에 맞추려 하지 말 것.

장애물피하기는 평소 `DODGE_TICK` 이 싣는 프레임과 같은 내용이라 기존 프레임 렌더러가 그대로
처리한다. 스냅샷을 뜨는 것은 게임 상태를 바꾸지 않는다(`DodgeGame.currentFrame()` 은 틱을 진행하지
않는다).

두 스냅샷 모두 **게임 모니터를 잡은 채로** 내보낸다. 상태를 읽고 모니터를 놓은 뒤에 내보내면 그
사이에 착수나 틱이 끼어들 수 있고, `GAME_SNAPSHOT` 은 방 전체로 가는 전체 상태라서 뒤늦게 도착한
낡은 스냅샷이 이미 반영된 수/틱을 지워 버린다. `RoomHub` 가 emit 을 방마다 직렬화하지만 페이로드는
그 락을 잡기 전에 계산되므로 도움이 되지 않는다.
...
**`departures` 는 v2 에서 추가됐고, 없으면 재생이 틀린다.** 이탈은 입력이 아니라 게임 상태를
직접 바꾸는 사건이라, 시드와 입력만으로는 재현되지 않는다 — 떠난 참가자가 재생 화면에서 계속
살아 피하고 있게 되어 승자와 길이가 원본과 달라진다. 그리고 이탈은 장애물피하기의 **주된 종료
경로**다. 재생기는 해당 틱을 진행하기 **전에**, 기록된 순서 그대로 이탈을 적용해야 한다.

`moves` 만 있는 줄은 v1 과 바이트 단위로 같다. 그래서 v1 리더는 v2 파일을 예외 없이 읽고
조용히 틀린 결과를 낸다 — 버전을 반드시 확인한다.

입력도 이탈도 없는 틱은 줄을 남기지 않는다. 3분짜리 8인 게임도 파일이 수십 KB에 그친다.
