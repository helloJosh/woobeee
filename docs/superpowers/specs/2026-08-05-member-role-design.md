...
> **개정 (2026-08-05)**: 사용자 요청으로 enum 값을 `MEMBER`/`ADMIN`에서
> `ROLE_MEMBER`/`ROLE_ADMIN`으로 변경했다. DB 저장 값과 V4의 DEFAULT/CHECK도 같은 형태가
> 되었고, 이름에 접두어가 포함되므로 `authority()` 변환 메서드는 제거하고 토큰에는
> `name()`을 그대로 싣는다. 아래 본문은 최초 승인 시점의 기록이다.
