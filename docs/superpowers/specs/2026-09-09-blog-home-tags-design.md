...
-1. **소개 배너**(`home-banner.tsx`, 9차) — 헤더 바로 아래 전체 폭 이미지(`public/home-banner.png`), 클릭하면 이력서
   (`public/resume.pdf`)가 새 탭으로 열린다. 두 파일은 개인 자료라 `.gitignore` 에 두고 배포 환경에서 넣는다. 이미지가
   없으면(404) 같은 문구("백엔드 개발자 김병우입니다")의 민트색 CSS 배너로 대체해 화면이 깨지지 않는다.
