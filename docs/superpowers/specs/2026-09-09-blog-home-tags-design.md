...
-1. **소개 배너**(`home-banner.tsx`, 9~10차) — 헤더 바로 아래 전체 폭 이미지 `public/home-banner.png`(3840×766, 우아한
   배너와 같은 구성: 민트 #0EEDD2 바탕에 「WOOBEEBLOG / 백엔드엔지니어 김병우 / 관심 있는 것을 공부합니다」).
   `scripts/home-banner.html` 을 헤드리스 크롬으로 찍어 만들었고 문구를 바꾸면 다시 찍는다. 배너 자체는 장식이고(13차) 이력서는 헤더 **RESUME 탭**(`components/resume-dialog.tsx`)이 **모달**로 연다 — 브라우저 PDF 뷰어(툴바·썸네일) 대신 `public/resume.png`(`scripts/render-resume.sh` 로 PDF 첫 장을 렌더) 한 장만 보인다(11차). 다운로드 링크는 없고(12차) 닫기 ×는 흰 종이 위에서 보이도록 어두운 원 배경이다. 이력서 PDF 는 사용자 결정으로 레포에 포함한다(공개 레포임을 알고 결정). 배너 이미지가 없으면(404) 같은 문구의 CSS 배너로 대체한다.
