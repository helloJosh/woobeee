...
-1. **소개 배너**(`home-banner.tsx`, 9~10차) — 헤더 바로 아래 전체 폭 이미지 `public/home-banner.png`(3840×766, 우아한
   ...
   가 **모달**로 열린다 — 브라우저 PDF 뷰어(툴바·썸네일) 대신 `public/resume.png`(`scripts/render-resume.sh` 로 PDF 첫 장을 렌더) 한 장만 보이고, 아래에 「PDF 다운로드」(`public/resume.pdf`) 링크만 둔다(11차). 이력서 PDF 는 사용자 결정으로 레포에 포함한다(공개 레포임을 알고 결정). 배너 이미지가 없으면(404) 같은 문구의 CSS 배너로 대체한다.
