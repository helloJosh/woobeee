...
- 헤더 탭: 기술블로그 · 일정(로그인 시). 게임 탭과 GitHub·이메일 아이콘은 2026-09-10 에 뺐다 — `/game` 은 URL 로만. 마이페이지는 아바타로 유지.
...
-1. **소개 배너**(`home-banner.tsx`, 9~10차) — 헤더 바로 아래 전체 폭 이미지 — 테마 팔레트로 찍은 두 장 `public/home-banner-light.png`(#3F72AF 바탕)·`-dark.png`(#0F4C75 바탕), `.dark` 로 CSS 가 고른다(14차, feature/theme-palette; 처음엔 민트 #0EEDD2 한 장이었다). 3840×766, 우아한
   배너와 같은 구성: 「WOOBEEEBLOG / 백엔드엔지니어 김병우 / 막힌 지점과 푼 방법을 기록합니다」).
   `scripts/render-banners.sh`(`scripts/home-banner-{light,dark}.html`)로 헤드리스 크롬에서 찍는다. 배너 자체는 장식이고(13차) 이력서는 헤더 **RESUME 탭**(`components/resume-dialog.tsx`)이 **모달**로 연다 — 브라우저 PDF 뷰어(툴바·썸네일) 대신 `public/resume.png`(`scripts/render-resume.sh` 로 PDF 첫 장을 렌더) 한 장만 보인다(11차). 다운로드 링크는 없고(12차) 닫기 ×는 흰 종이 위에서 보이도록 어두운 원 배경이다. 이력서 PDF 는 사용자 결정으로 레포에 포함한다(공개 레포임을 알고 결정). 배너 이미지가 없으면(404) 같은 문구의 CSS 배너로 대체한다.
0. **상단 태그 알약 줄**(`tag-bar.tsx`, 3차) — 사용자가 우아한 홈의 상단 알약 내비를 가리키며 "태그로 구분"을 요청. 인기 태그 50개를 작은 알약(text-xs)으로 **전체 폭 가운데**에 두고(6차, 사용자 캡처 "딱 이렇게"), 그 아래 큰 여백 뒤 두 열(글 목록·사이드바)이 같은 높이에서 시작한다. 컨테이너는 max-w-4xl, 사이드바 200px. 첫 칸 「-」 = 전체(태그 해제), 활성 태그는 채움. 사이드바 「태그」는 상위 20개에 글 수를 함께 보인다.
1. **왼쪽 — 글 세로 목록**(`post-list-item.tsx`): 한 글 = `2026. 09. 08.` + 카테고리명(작성자 필드가 없어 대신) →
   큰 제목(2xl/3xl, 링크) → 작성자가 적은 설명(없으면 생략, 5차 — 본문 요약 표시는 폐기). 글 아래 태그 chips 는 7차에서 제거(태그는 상단 알약 줄·사이드바에서만). 항목 사이 구분선. 무한 스크롤(`useInfinitePosts`, pageSize 10,
   `tag` 파라미터). 이미지·썸네일 없음.
2. **오른쓽 — 사이드바**(`home-sidebar.tsx`, `lg` 이상 200px·sticky). 휴대폰 폭(`lg` 미만)에서는 태그 알약 줄과 사이드바를 숨기고 목록만 보인다(8차, 우아한 모바일 캡처) — 목록 위 오른쪽 「≡ 카테고리 · 태그」 버튼이 오른쪽 서랍(`Sheet`)으로 같은 내용을 연다:
   - 「카테고리」 — 헤더 클릭으로 접고 펼침. **기본 접힘**, 상태는 `localStorage("home.categoriesOpen")`. 접힌 채로도
     선택된 카테고리 이름을 헤더 옆에 보인다. 펼치면 「전체」 + 부모(글 수). 자식이 있는 부모는 오른쪽 `>`/`v` 로 자식 목록을 따로 접고 펼치며, 고른 카테고리의 부모는 자동으로 펼쳐진다
