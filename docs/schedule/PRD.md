...
| SCHEDULE-AC-34 | 할 일 시간 입력 | `startTime`/`endTime`(`HH:mm`, 선택)을 `tasks.start_time`/`end_time`(TIME, V11)에 저장·응답. 해당 날짜가 없으면 시간은 null 로 정규화. 같은 날짜에 둘 다 있고 `endTime < startTime` 이면 400 + `schedule_invalidDateRange`. 날짜 컬럼과 달력·자동 완료·다이제스트 로직은 그대로. 트리 행·달력 막대 라벨에 시간이 있으면 표시(`formatTaskRange`). 서버가 `LocalTime` 을 `10:00:00` 처럼 초까지 내려도 프론트 `normalizeTree` 가 `HH:mm` 으로 잘라 화면 계약을 지킨다 |
...
| SCHEDULE-AC-43 | 트리 정렬 | **API 가 정렬한다** — `GET /tree` 의 할 일·마일스톤 조회 쿼리가 `ORDER BY start_date ASC NULLS LAST, sort_order, id`. 프로젝트 순서는 `sort_order, id` 그대로. 서비스의 부모별 묶음은 이 순서를 보존하고 프론트는 받은 순서를 그대로 그린다(달력 lane 도 같은 순서). 처음엔 프론트 `sortTreeByStart` 였으나 사용자 요청으로 서버로 옮겼다 |
