...
| BLOG-AC-13 | 조회 응답의 이미지 URL 은 presigned URL 이며 presigner 가 준 문자열을 그대로 박는다(서명이 host·키를 포함하므로 후처리 금지). 서명 대상 키는 `{postId}/{basename}` 으로, 플레이스홀더의 경로 성분은 제거한다 — 안 그러면 같은 버킷의 `profiles/` 를 여는 유효한 URL 이 만들어진다. 파일명은 미리 인코딩하지 않는다(이중 인코딩) | `PostServiceImplTest` |
