...
public class DodgeReplayRunner {
    /**
     * 손상된 기보가 무한 루프에 빠지지 않도록 하는 안전장치. 스폰 확률은 틱 750에서 최댓값
     * (0.20)에 도달한 뒤 더 오르지 않고, 격자 높이(48서브행)상 장애물 하나가 바닥까지 내려오는 데
     * 47틱이면 충분하다 — 규칙 자체에는 "정상 게임"의 길이를 못박는 상한이 없다(밀도가 1.0
